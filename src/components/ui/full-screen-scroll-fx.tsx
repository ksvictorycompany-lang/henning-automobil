...
export const FullScreenScrollFX = forwardRef<HTMLDivElement, FullScreenFXProps>(
  (
    ...
    const cssVars: CSSProperties = {
      ...
      ["--fx-text" as string]: colors.text ?? "rgba(234,234,238,0.95)",
      ["--fx-overlay" as string]: colors.overlay ?? "rgba(0,0,0,0.45)",
      ["--fx-page-bg" as string]: colors.pageBg ?? "#1C1C1E",
      ["--fx-stage-bg" as string]: colors.stageBg ?? "#1C1C1E",
      ["--fx-gap" as string]: `${gap}rem`,
      ["--fx-grid-px" as string]: `${gridPaddingX}rem`,
      ["--fx-row-gap" as string]: "10px",
    };

    const sectionHeight = `${Math.max(2, total + 1) * 100}vh`;
    ...
    return (
      <div
        ...
        className={[styles.fx, className].filter(Boolean).join(" ")}
        style={{ ...cssVars, ...style }}
        aria-label={ariaLabel}
      >
        <div className={styles.fxScroll}>
          <div
            className={styles.fxFixedSection}
            ref={fixedSectionRef}
            ...
            <div className={styles.fxFixed} ref={fixedRef}>
              ...
              <div className={styles.fxGrid}>
                ...
                <div className={styles.fxContent}>
                  {/* Left */}
                  <div className={styles.fxLeft} role="list">
                    <div className={styles.fxTrack} ref={leftTrackRef}>
                      {sections.map((s, i) => (
                        <div
                          key={`L-${s.id ?? i}`}
                          className={`${styles.fxItem} ${styles.fxLeftItem}${i === index ? " active" : ""}`}
                  ...
                  <div className={styles.fxCenter}>
                    {sections.map((s, sIdx) => {
                      ...
                      return (
                        <div
                          key={`C-${s.id ?? sIdx}`}
                          className={`${styles.fxFeatured}${sIdx === index ? " active" : ""}`}
                        >
                          <h2 className={styles.fxFeaturedTitle}>
                  ...
                  {/* Right */}
                  <div className={styles.fxRight} role="list">
                    <div className={styles.fxTrack} ref={rightTrackRef}>
                      {sections.map((s, i) => (
                        <div
                          key={`R-${s.id ?? i}`}
                          className={`${styles.fxItem} ${styles.fxRightItem}${i === index ? " active" : ""}`}
